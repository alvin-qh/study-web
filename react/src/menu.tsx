import { type IconName, type MaybeElement, Menu, MenuItem } from '@blueprintjs/core';
...
class MenuData {
  ...
  constructor(basePath: string, items: MenuItemData[]) {
  ...
  render(navigate: NavigateFunction): JSX.Element {
    return (
      <Menu>
        {
          this.items.map((item) => (
...
export const BasicMenu = (): JSX.Element => {
...
export const HookMenu = (): JSX.Element => {
  const navigate = useNavigate();
...
export const ReduxMenu = (): JSX.Element => {
  const navigate = useNavigate();
