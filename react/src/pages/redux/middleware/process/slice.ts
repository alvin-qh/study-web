import { type AnyAction, createAsyncThunk, createSlice, type Dispatch, type PayloadAction } from '@reduxjs/toolkit';
 ...
 */
const delayNumber = async (milliseconds: number, value: number): Promise<number> =>
 ...
 */
const delayIncrease = createAsyncThunk('processor/delayIncrease', async (payload: number, thunkAPI): Promise<number> => await delayNumber(2000, payload));

/**
 * 通过 createAsyncThunk 函数创建异步 Action
 */
const delayDecrease = createAsyncThunk('processor/delayDecrease', async (payload: number, thunkAPI): Promise<number> => await delayNumber(2000, payload));
...
const slice = createSlice({
  ...
  extraReducers(builder) {
     ...
     */
    builder.addCase(delayIncrease.pending.type, (state: State, action: PayloadAction<number>): void => {
      state.waiting = false;
      state.value += action.payload;
      if (state.value >= 1) {
        state.value = 1;
