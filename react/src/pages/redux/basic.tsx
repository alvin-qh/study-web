 ...
 *                    () => todoAppStore.dispatch(toggleTodo(todo.id))
 *                } />
 *              ))
 *            }
 *            </ul>
 *          );
 *
 *      上述这种方法需手动处理 state => props 和 dispatch => props，所以 redux-react 提供了 connect 函数，返回一个 connector,
 * 包装一个自动完成上述转换的组件，避免手动传递 props 和处理事件，例如：
 *
...
import configStore from './basic/reducer';
...
const ReduxBasic = (): JSX.Element => (
  <Provider store={store}>
    <div className="w-1/3 px-2 py-4">
      <p className="font-medium text-lg mb-2">Redux React Todo List: </p>
      <div>
        <AddTodo />
      </div>
      <div className="mt-4">
