...
import { type JSX, memo, useEffect, useMemo, useReducer, useState } from 'react';
 ...
 * 简易计算器组件
 */
const Calculator = memo((): JSX.Element => {
   ...
   */
  const selectItemRenderer =
    useMemo(() => (item: IOperator, { handleClick }: ItemRendererProps): JSX.Element => (
      <MenuItem
        label={item.operator}
    ...
    <div>
      ...
      <div className="flex flex-row space-x-2 items-center mt-2">
        ...
        <Button
          ...
          onClick={
            () => {
              /* 点击按钮，通过 dispatch 调用 reduce 计算结果 */
              dispatch({ firstNum, secondNum, operator: selectedOperator.operator });
            }
          }
        />
