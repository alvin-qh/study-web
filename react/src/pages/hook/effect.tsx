...
class EffectWithClass extends PureComponent<EffectProp, EffectState> {
   ...
   */
  componentDidMount() {
    this.timer = setInterval(() => { this.setState({ ...this.state, count: this.state.count + 1 }) }, 1000);
  }
  ...
  render(): React.JSX.Element {
    return (
      <div className="px-2 space-y-2 mt-2 flex flex-col items-start">
        <p className="text-lg">The count value is: <b className="text-2xl font-medium text-red-500">{this.state.count}</b></p>
      </div>
    );
  }
}
 ...
 */
const EffectHookWithFunc = ({ ...props }: EffectProp): React.JSX.Element => {
...
const HookEffect = (): React.JSX.Element => {
  return (
    <div className="px-4 py-6 space-y-6">
      <div>
        <h1 className="text-lg font-medium px-2 py-1 bg-gradient-to-r from-sky-100 to-sky-50">Effect In Component Class: </h1>
        <EffectWithClass />
      </div>
      <div>
        <h1 className="text-lg font-medium px-2 py-1 bg-gradient-to-r from-rose-100 to-rose-50">Effect Hook In Component Function: </h1>
        <EffectHookWithFunc />
      </div>
    </div>
  );
