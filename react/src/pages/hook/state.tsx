  ...
  render(): JSX.Element {
    return (
      <div className="px-2 space-y-2 mt-2 flex flex-col items-start">
        <p className="text-lg">You clicked <b className="text-2xl text-red-500">
          {this.state.count /* 读取 state 中的 count 值 */}</b> times
        </p>
        <Button
...
const StateHookWithFunc = ({ ...props }: StateProp): JSX.Element => {
  ...
  return (
    <div className="px-2 space-y-2 mt-2 flex flex-col items-start">
      ...
      <Button
        onClick={() => { setCount((c) => c + 1); } /* 通过 setCount 设置 state 值 */}
        className="font-medium"
        large={true}
