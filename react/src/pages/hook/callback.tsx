...
const Counter = memo(({ initial, onChange }: CounterProps): JSX.Element => {
  // 计数器数值 state
  const [, setCounter] = useState<number>(parseInt((initial * 10).toFixed(0), 10));
  ...
  const handleClickIncrement = useCallback(handleClick(1), []);
  const handleClickDecrement = useCallback(handleClick(-1), []);
...
const HookCallback = (): JSX.Element => {
  ...
  // 设置表格行数的 state
  const [rows, setRows] = useState<number[]>([...Array(10)].map(() => random(1, true)));
