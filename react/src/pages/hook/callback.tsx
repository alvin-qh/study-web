 ...
 * 属性设置组件
 */
const Setting = ({ onChange, howManyFuncs = 0 }: SettingProps): React.JSX.Element => (
 ...
 */
const Counter = memo(({ initial, onChange }: CounterProps): React.JSX.Element => {
  // 计数器数值 state
 ...
 * Callback 组件
 */
const HookCallback = (): React.JSX.Element => {
   ...
   * 渲染表格第一列单元格
   */
  const counterCellRenderer = (rowIndex: number): React.JSX.Element => {
    return (
   ...
   * 渲染表格第二列单元格
   */
  const operatorsCellRenderer = (rowIndex: number): React.JSX.Element => {
    return (
