 ...
 */
const Welcome = ({ name, gender, ...props }: ConditionProp<HTMLDivElement>): React.JSX.Element => {
  // 根据分支语句进行条件渲染
  /*
...
const Gender = ({ name, gender, ...props }: ConditionProp<HTMLDivElement>): React.JSX.Element => (
  <p {...props}>
    {name}"s gender is: {(gender === "M" && <b>Male</b>) || (gender === "F" && <b>Female</b>)}
  </p>
)

const ConditionRender = (): React.JSX.Element => {
  // 定义一组 state hook
