  ...
  <div class="component-event">
    <form @submit="">
      <div>
        ...
        <input type="number" placeholder="0" v-model="number1">
      </div>
      <div>
      ...
      <div>
        ...
        <input type="number" placeholder="0" v-model="number2">
      </div>
      <div>=</div>
      <div>
...
import { operators, type Operator, type ModelType, type CalculateEvent } from './ComponentEvent';
...
function emitEvents(result: number) {
  ...
  if (props.modelValue) {
    ...
    // 更新 `modelValue` 属性值
    mv.number1 = number1.value;
    mv.number2 = number2.value;
    mv.operator = opt.value;
    mv.result = result;
