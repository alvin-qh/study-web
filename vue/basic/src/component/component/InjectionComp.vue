...
-->
<template>
  <AttributeList title="Global" :attrs="globalAttrs" />
  <AttributeList title="Local" :attrs="localAttrs" />
  <AttributeList title="Point" :attrs="((clientPoint || {}) as Attributes)" />
</template>
...
import { type InjectionObj } from './InjectionComp';

// 定义响应式变量, 用于获取所有全局注入值
const globalAttrs = ref<Attributes>({
  // 表示名为 `global` 的全局注入值
  global: null
});

// 定义响应式变量, 用于获取所有父组件注入值
const localAttrs = ref<Attributes>({
...
const { clientPoint, changeClientPoint } = inject<InjectionObj>(
  'reactive-object',
  {
    clientPoint: undefined,
    changeClientPoint: undefined
  }
);
