...
> 设置npm国内代理: `$ npm config set registry https://registry.npm.taobao.org`  

## 目录结构

- asset_src: css与javascript源代码
  - css: 样式表源码
  - js: Javascript源码
- asset_build：存放编译css和javascript产生的中间文件
- bower_components: 由bower下载的内容，包括`require.js`, `jquery`, `bootstrap`以及`pasley`等
- node_modules: 由npm下载的内容，包括`gulp`以及其插件
- www: 网址项目目录
  - asset: 经过`gulp`编译打包后的css, javascript以及图片等内容

## 打包

打包主要是通过`gulp`来进行，分为：初始化、打包和发布这几个大步骤。

### 初始化

初始化的目的是将部分不支持`AMD`的javascript脚本进行处理：
```javascript
gulp.task('init', () => {
    return gulp.src(['asset_src/js/vendor/parsley-i18n.js'])
        .pipe(rjsOpt(() => {
            return {
                optimize: 'none',
                useStrict: true,
                ...
                paths: {
                    'parsley-i18n': '../asset_src/js/vendor/parsley-i18n',
        ...
        .pipe(concat({path: 'parsley.js', cwd: ''}))
        .pipe(gulp.dest('asset_build/js/vendor'))
});
```
上述代码主要初始化的内容是`parsley.js`，目的是将`parsley.js`与`parsley-i18n.js`两个文件打包在一起，令`parsley-i18n.js`可以正确的依赖于`parsley.js` 。

由于`baseUrl`设置为`bower_components`，所以必须指定`parsley-i18n.js`文件相对于`bower_components`目录的路径，由于`parsley-i18n`依赖于`parsley`和`jQuery`，所以也得指定这两个文件相对于`baseUrl`的位置。

打包完毕后输出`parsley.js`文件，输出到`asset_build/js/vendor/`路径下。 

### 打包常用 Javascript：

某些Javascript脚本几乎在每个页面中被使用，这些脚本可以整合打包在一起，在页面中直接加载，一方面可以及时缓冲这部分脚本，令页面加载更快速，另一方面，页面中的其它脚本可以很方便的使用它们。
这部分打包带吗如下：

```javascript
gulp.task('js-ie8-support', () => {
    return gulp.src([
        'bower_components/html5shiv/dist/html5shiv.js',
        'bower_components/respond/dest/respond.min.js'
    ])
        .pipe(concat({path: 'ie8-support.js', cwd: ''}))
        .pipe(ifElse(IS_PROD, uglify))
        .pipe(gulp.dest('asset_build/js/vendor'));
});
...
gulp.task('js-vendor-bundle', () => {
    return gulp.src([
        ...
        'asset_build/js/vendor/parsley.js',
        'bower_components/bootstrap/dist/js/bootstrap.js',
        'asset_src/js/config.js'
    ])
...
上述代码中，`js-ie8-support`任务打包了两个文件，主要是为了对早期浏览器进行支持；

`js-vendor-bundle`任务则打包了`require.js`，`jquery.js`，`parsley.js`，`bootstrap.js`以及`require.js`的配置文件`config.js`。其中：`parsley.js`是在`init`任务中生成的，所以位于`asset_build/js/vendor`目录下，而`config.js`是为当前项目编写的，所以位于`asset_src/js`目录下。

`config.js`配置了`require.js`加载 Javascript 的方式和路径，配置如下：
...
该配置表示，`require.js`默认加载 Javascript 的路径为 `asset/js/vendor`，即`www/asset/js/vendor`，位于该目录下的 Javascript 文件，均可以文件名作为ID进行加载；对于`asset/js/common`目录下的 Javascript 文件，则可以使用`common/xxxx`格式的ID进行加载。

如果仍有其它特殊加载的 Javascript，则需要在`paths`配置项中进一步说明，例如：

```javascript
paths: {
  	'jquery': '//cdn.bootcss.com/jquery/3.1.1/jquery.js'
}
```

表示ID为`jquery`的模块从指定网址进行加载。

打包完成后，产生`vendor.js`文件，将该文件直接包含于html中即可：
...
完成上述操作后，所需的 Javascript 文件均已经产生，此时使用`require.js`，已经可以正确的加载指定 Javascript 模块了，但会造成网页频繁发送加载 Javascript 的请求，此时应当将一个页面所需的 Javascript 打包整合在一起。

```javascript
gulp.task('rjs', () => {
    return gulp.src([
        '!asset_build/js/vendor/**/*.js',
        '!asset_build/js/common/**/*.js',
        '!asset_build/js/config.js',
        'asset_build/js/**/*.js'
    ]).pipe(rjsOpt(() => {
        return {
            optimize: 'none',
            useStrict: true,
            baseUrl: 'asset_build/js',
            paths: {
    ...
    })).pipe(gulp.dest('asset_build/js'));
});
```

上述代码表示：将`asset_build/js`目录下除过`vendor`目录, `common`目录和`config.js`以外的 Javascript 文件进行优化，优化通过`gulp-requirejs-optimize`插件完成，将每个 Javascript 模块所需的依赖合并到该模块所在的文件中。合并时的依赖项处理方法为：

- 所有依赖项默认路径为`asset_build/js`，但`jquery.js`, `bootstrap.js`以及`parsley.js`依赖项从`bower`下载路径加载。
- 所有依赖ID为`common/xxx`的，从`asset_build/js/common`目录加载；
- 所有依赖ID为`vendor/xxx`的，从`asset_build/js/vendor`目录加载。
- 打包的文件中，忽略`jquery`，`bootstrap`以及`parsley`这三个模块（这三个模块已经打包为一个文件并由浏览器预先加载）


注意，这里的配置和`config.js`中的配置略有不同，这主要是因为`baseUrl`配置的差异造成的。`config.js`指定的默认加载路径是`www/asset/js`。`gulp`中配置的`baseUrl`为`asset_build/js`中间目录，因为此时代码还未发布到`www/asset/js`目标目录中。

所以，要进行优化时，加载`asset_build/js/vendor`目录下的模块必须以`vendor/xxx`这样的ID来加载，而在运行时，则直接通过 Javascript 文件名作为ID，即可加载`www/asset/js/vendor`目录下的模块。

要令某些模块在运行时来动态加载，则需要在`exclude`配置项中指定这些模块，保证这些模块不要打包到目标 Javascript 文件中。

### 发布

发布即将指定文件从`asset_build`路径发布到`www/asset`目录下，发布的内容由`gulp`脚本控制：

```javascript
gulp.task('manifest', () => {
    return gulp.src([
        '!asset_build/js/common/**',
        '!asset_build/js/vendor/parsley*.js',
	    '!asset_build/js/vendor/bootstrap*.js',	// 如果要动态加载该模块，则删除这一行
        'asset_build/**'
    ])
        .pipe(gulp.dest('www/asset'));
});
```

这段代码没有为文件增加`md5后缀`，在实际生产环境应该加上。

代码中采用了`bootstrap-notify`组件（http://bootstrap-notify.remabledesigns.com/），是从官网下载并放入`asset_src/js/vendor`路径下，如果该模块在打包过程中已经和目标代码打包在一起，则无需发布到最终目录。如果有些页面需要动态加载该模块，则删除`!asset_build/js/vendor/bootstrap*.js`，让其发布到`www/asset/js/vendor`目录中，以便其它模块可以动态加载。

如果模块在打包时包含指定模块，则代码为：

```javascript
require(['jquery', 'parsley', 'vendor/bootstrap-notify'], function ($) {
	......
});
```

`vendor/bootstrap-notify`表示从`asset_build/js/vendor`目录下加载`bootstrap-notify`模块。

如果模块在运行时包含指定模块，则需要在优化 Javascript 模块的步骤中排除该模块，代码为：
...
此时`bootstrap-notify`模块将会从服务端动态加载。
