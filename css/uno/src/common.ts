...
export class Story {
  ...
  constructor(title: string, describe: string = '', language: string = 'html') {
    ...
    this.$wrapper.className = 'border-solid border-gray-1 rounded-sm py-2 px-2 shadow-xl my-5 w-full';
    ...
    $storyTitle.className = 'font-bold text-sm md:text-lg m-y-0';
    ...
    if (language === 'html') {
      ...
      $btnSx.className = 'border border-gray px-2 py-1 font-semibold text-xs w-10';
      $btnSx.innerText = 'SX';
      $htmlBlock.appendChild($btnSx);

      const $btnMd: HTMLButtonElement = document.createElement('button');
      $btnMd.className = 'border border-gray px-2 py-1 font-semibold text-xs ml-3 w-10';
      $btnMd.innerText = 'MD';
  ...
  code(code: string): Story {
    ...
    if (lang === 'markdown') {
      this.$code.innerHTML = new showdown.Converter({ tables: true }).makeHtml(code);
    } else {
