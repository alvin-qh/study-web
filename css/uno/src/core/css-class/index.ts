...
window.onload = () => {
  const book = new StoryBook('Use UnoCss Class', true);
  book.append([
    new Story('Auto Columns', '使用 `auto-cols-{size}` 来设置网格列的大小')
      .code(`\
<div class="p-4 bg-gradient-to-r from-light-blue-50 to-light-blue-100">
  <div class="p-6 max-w-sm mx-auto bg-white rounded-xl shadow-md flex items-center space-x-4">
    ...
    <div>
      ...
      <p class="text-gray-500 m-y-1">You have a new message!</p>
    </div>
  </div>
</div>
<!--
  使用 UnoCss 的 'flexbox' 和 'padding' 功能类 ('flex', 'flex-shrink-0', 和 'p-6') 来控制整体的卡片布局
  ...
  <div class="py-8 px-8 max-w-sm mx-auto
    ...
    <div class="text-center space-y-2 sm:text-left">
      <div class="space-y-0.5">
        <p class="text-lg text-black font-semibold m-y-2">
          Erin Lindford
        </p>
        <p class="text-gray-500 font-medium m-y-2">
          Product Engineer
        </p>
      </div>
...
<div class="p-6 text-center bg-gradient-to-r from-green-50 to-green-100">
  ...
  <button class="py-2 px-4 font-semibold rounded-lg
                            ...
                            border-0 font-size-4
                            bg-green-500 hover:bg-green-700">
    Click me
  </button>

  <!-- Extracting classes using @apply -->
  <button class="btn btn-green m-l-2">
    Button
  </button>
</div>
...
<style type="text/css">
  .btn {
    @apply py-2 px-4 font-semibold rounded-lg shadow-md border-0 font-size-4;
  }
  .btn-green {
